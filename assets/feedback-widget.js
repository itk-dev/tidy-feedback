...
const pickElement = () => {
    ...
    const mouseoverHandler = (event) => {
        ...
        if (lastElement !== event.target) {
            ...
            xEnd.style.left =
                lastElement.offsetLeft + lastElement.offsetWidth + "px";

            yStart.style.top = lastElement.offsetTop + "px";
            yEnd.style.top =
                lastElement.offsetTop + lastElement.offsetHeight + "px";
        }
    };
    ...
    const clickHandler = async (event) => {
        ...
        if (lastElement === event.target) {
            ...
            blob.text().then((svg) => addData("svg", svg));
        }
    };
...
const selectRegion = () => {
    ...
    const mouseupHandler = async (event) => {
      ...
      const data = {};
        ...
        const form = document.querySelector(".tidy-feedback-form");
        if (form) {
              ...
              console.log(data)
                fetch(form.action, {
                    ...
                    if (201 === response.status) {
                      ...
                      response.json()
                        .then(data =>                     showMessage('Error creating feedback: '+JSON.stringify(data)))
                    }
